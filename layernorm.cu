...
__global__ void layer_norm(float* x, float* y, float g, float b, int row, int col) {
    ...
    if (tid == 0) s_mean = sum / (float) col;
    //一个block里所有线程同步
    __syncthreads();
    float variance = (value - s_mean) * (value - s_mean);
    variance = block_reduce_sum(variance);
    if (tid == 0) s_variance = rsqrtf(variance / (float) col + epsilon);
    //一个block里所有线程同步
    __syncthreads();
    
    if (idx < row * col) y[idx] = ((value - s_mean) * s_variance) * g + b;
}



__global__ void layer_norm_float4_block(float* x, float* y, float g, float b, int N, int K) {
    ...
    float value = (idx < N * K) ? (tmp_x.x + tmp_x.y 
                                + tmp_x.z + tmp_x.w) : 0.0f;
    float sum = block_reduce_sum(value);
    ...
    float variance = tmp_x_hat.x * tmp_x_hat.x 
...
__global__ void layer_norm_float4_grid(float* x, float* y, float g, float b, int N, int K) {
    ...
    if (lane == 0) s_mean[warpID] = value / (float) K;
    __syncthreads();
    ...
    if (lane == 0) s_variance[warpID] = rsqrtf(variance / (float) K + epsilon);
    __syncthreads();

    // if(idx == 0){
    //     printf("idx :%d, value :%f, s_mean :%f, s_variance :%f\n", idx, value, s_mean, s_variance);
    // }
