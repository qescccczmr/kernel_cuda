...
//nvcc Sgemv_k32.cu -o Sgemv_k32 -lcublas
...
__global__ void Sgemv_k32(float* A, float* x, float* y, const int M,const int K) {
    ...
    int current_row = blockDim.y * blockIdx.x + threadIdx.y; // 4*(M/4 -1) +(0~3)

    if(current_row < M){
        float res=0;
        int kIteration = K/warp_size; //也可以处理k=64,96的情况，128有其它的实现方式
