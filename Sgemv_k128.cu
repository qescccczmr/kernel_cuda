...
//nvcc Sgemv_k128.cu -o Sgemv_k128 -lcublas
...
//dim3 dimGrid(M/4);
//dim3 dimBlock(32,4);
__global__ void Sgemv_v128(float* A, float* x, float* y, const int M, const int K) {
    ...
    if(current_row < M){
        ...
        for(int i=0; i< kIteration; i++){
            ...
            float4 current_val= reinterpret_cast<float4 *>(&A[current_row*K])[current_col_vec];
...
int main(int argc, char** argv) {
    ...
    for (int run = 0 ; run < iteration; run ++ ) {
        dim3 dimGrid(M/4);
        dim3 dimBlock(32,4);
        Sgemv_v128<<< dimGrid, dimBlock >>>(d_A, d_x, d_y, M, K);
    }
