...
__global__ void sgemm(float* gloabl_a, float* global_b, float* global_c, int M, int N, int K) {
  ...
  */
  ...
  __shared__ float shared_a[block_m][block_k], shared_b[block_k][block_n]; 
  ...
  */
  ...
  if (gm_am >= M || gm_bn >= N) return;
  
  float sum = 0.f;
  for (int bk = 0; bk < (K + block_k - 1) / block_k; ++bk) {//大迭代，负责a中一行/b中一列的结果

    int a_offset = gm_am * K + bk * block_k + sm_ak;//MxK矩阵，第gm_am行，第bk * block_k + sm_ak列

    shared_a[sm_am][sm_ak] = gloabl_a[a_offset];//32/x32

    int b_offset = (bk * block_k + sm_bk) * N + gm_bn;//KxN矩阵，第bk * block_k + sm_bk行，第gm_bn列

    shared_b[sm_bk][sm_bn] = global_b[b_offset];//32x32

    __syncthreads();
    #pragma unroll
    for (int k = 0; k < block_k; ++k) {//小迭代，负责BK内部a的一行/b的一列的求和
      sum += shared_a[sm_am][k] * shared_b[k][sm_bn];//操作的数据都在shared memory当中
    }
    __syncthreads();
  }
  
  global_c[gm_am * N + gm_bn] = sum;//把结果sum写回c中索引是load_gmem_a_m * N + load_gmem_b_n的地方
}
