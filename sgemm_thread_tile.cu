...
//nvcc sgemm_thread_tile.cu -lcublas -o sgemm_thread_tile
#define WARP_SIZE 32
...
__global__ void sgemm_thread_tile_vec4(
  float* global_a, float* global_b, float* global_c, int M, int N, int K) {         
  /*
  ...
  */
  ...
  int tx = threadIdx.x;
  int ty = threadIdx.y;
  int tid = threadIdx.y * blockDim.x + tx; // 每个线程块内部的线程索引
  __shared__ float shared_a[block_m][block_k], shared_b[block_k][block_n]; // 2*128*8*4=8KB
  
  int sm_am = tid / 2;
  int sm_ak = (tid % 2 == 0) ? 0 : 4;
  /*
  针对线程块block里某一个编号是tid的线程，计算它负责的元素在a的shared memory里的索引
  shared_a的形状是128x8，128行，每行8个元素，每个线程需要读取4个元素，一行需要2个线程，总共128行，需要的线程数刚好是2x128=256
  ...
  */
  ...
  shared_b的形状是8x128，8行，每行128个元素，每个线程需要读取4个元素，一行需要32个线程，总共8行，需要的线程数刚好是8x32=256
  ...
  */
  ...
  float r_c[thread_m][thread_n] = {0.0}; // 8x8个元素，这些元素是用寄存器保存的
  ...
  #pragma unroll
  for (int m = 0; m < thread_m; ++m) {
    int gm_cm = by * block_m + ty * thread_m + m;//当前线程负责写入元素在c矩阵(全局内存)中的行索引
    #pragma unroll
    for (int n = 0; n < thread_n; n += 4) {
      int gm_cn = bx * block_n + tx * thread_n + n;////当前线程负责写入元素在c矩阵(全局内存)中的列索引

      FLOAT4(global_c[gm_cm * N + gm_cn]) = FLOAT4(r_c[m][n]);////采用float4向量化写入
    }
  }
}
