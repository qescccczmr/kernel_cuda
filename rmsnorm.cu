...
// y=y'*g (g: scale)  
__global__ void rms_norm(float* x, float* y, float g, float b, int N, int K) {
    ...
    __shared__ float s_variance; // 一个block里共享的变量
    float value = (idx < N * K) ? x[idx] : 0.0f; // 只加载一次
...
__global__ void rms_norm_float4_block(float* x, float* y, float g, float b, int N, int K) {
    ...
    float variance = (idx < N * K) ? (tmp_x.x * tmp_x.x 
                                    + tmp_x.y * tmp_x.y 
                                    + tmp_x.z * tmp_x.z 
    ...
    if (idx < N * K) FLOAT4(y[idx]) = tmp_y;
}

__global__ void rms_norm_float4_grid(float* x, float* y, float g, float b, int N, int K, float* row_mean) {
    ...
    if(current_row < N){
        ...
        for(int i = 0; i < kIteration; i++){
            ...
            float4 current_x = reinterpret_cast<float4 *>(&x[current_row*K])[current_col_vec];
        ...
        if(laneId == 0) row_mean[current_row] = rsqrtf(res / (float) K + epsilon);
        ...
        for(int i = 0; i < kIteration; i++)
        {
            ...
            reinterpret_cast<float4 *>(&y[current_row*K])[current_col_vec] = current_y;
        }
    }
...
int main(){
    ...
    //rms_norm_float4_grid<<<CeilDiv((int)N/4, reduce_dim),block_size>>>(d_A, d_B, g, b, row, reduce_dim);
